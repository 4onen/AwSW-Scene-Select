...
def block_replay_over_mod_chapter_boundaries(ml,fss):
    fss.end_replay_at_ml_node(ml.find_label('_mod_fixjmp'))

def link_test_environment(ml,fss):
...
def link_minigames(ml,fss):
    ...
    link_bryce_drinking_game(ml,fss)


def link_endings(ml,fss):
...
class MyAwSWMod(Mod):
    ...
    def mod_load():
        ...
        block_replay_over_mod_chapter_boundaries(ml,fss)

        link_test_environment(ml,fss)

        link_minigames(ml,fss)
        # link_endings(ml,fss)
        link_eck_minigames(ml,fss)
